...
const getButtonThemesByVariant = (type, globalOverrides) => {
  ...
  const buttonVariantSchema = globalOverrides.buttonVariant;
  ...
  return (0, _reduce.default)(_context = (0, _keys.default)(variantThemes)).call(_context, (prev, current) => {
    ...
    const variantName = type !== 'plain' ? (0, _concat.default)(_context2 = "".concat(type, "-")).call(_context2, current) : current;
    const currentTheme = variantThemes[current];

    const vTheme = _objectSpread(_objectSpread({}, prev), {}, {
      [variantName]: {
...
const buttonThemes = (variantTypes, globalOverrides) => {
  ...
  const buttonVariantSchema = globalStyles.buttonVariant;

  if (!variantTypes) {
    var _context3;

    // return all button variants
    return (0, _reduce.default)(_context3 = (0, _keys.default)(buttonVariantSchema)).call(_context3, (prev, current) => _objectSpread(_objectSpread({}, prev), getButtonThemesByVariant(current, globalStyles)), {});
  }

  if ((0, _isArray.default)(variantTypes)) {
    // return button variants based on indicated types.
    return (0, _reduce.default)(variantTypes).call(variantTypes, (prev, current) => _objectSpread(_objectSpread({}, prev), getButtonThemesByVariant(current, globalStyles)), {});
  }
